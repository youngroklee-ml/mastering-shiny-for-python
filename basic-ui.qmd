...
::: {.callout-note}
While you can return just `mtcars` in `dynamic()` for default interactive table, to enable filters, either `render.DataGrid()` or `render.DataTable()` needs to be returned with `fitler=True`.
:::
