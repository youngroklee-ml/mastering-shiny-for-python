...
:::{.callout-note}
I used `is_string_dtype()` to check whether the column is for categorical variable because the `Species` column in `iris` data has not been converted to categorical variable yet. It would be better to consider converting columns to Categorical dtype first and use a function `is_categorical_dtype()` or its equivalent.
:::
